...
export class CheckpointSystem {
  ...
  private lastCheckpointReached: number = -1; // Prevent double-triggering
  ...
  public update(
    ...
    // Check if player reached checkpoint (prevent double-triggering)
    if (
      this.currentCheckpointIndex !== this.lastCheckpointReached &&
      currentCheckpoint.checkPlayerCollision(playerCar.getPosition())
    ) {
      console.log(
      ...
      this.lastCheckpointReached = this.currentCheckpointIndex;
      ...
      // Reset game timer for next checkpoint (extend time based on checkpoint)
      this.gameTimer = 0;

      return "checkpoint";
    }

    // Check timeout against current checkpoint's time limit
  ...
  public addTimeBonus(seconds: number): void {
    // Subtract time from game timer to give more time
